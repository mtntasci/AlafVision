          ...
          {isStreaming && (
            <svg className="absolute inset-0 w-full h-full z-20 pointer-events-none" viewBox={`0 0 ${videoDimensions.width} ${videoDimensions.height}`} preserveAspectRatio="xMidYMid meet">
              {results.map((res) => {
                ...
                const boxColor = isFighting ? "text-red-500" : (knownName ? "text-purple-500" : "text-accent");
          ...
          <div className="flex-1 overflow-y-auto custom-scrollbar pr-2 flex flex-col gap-2">
            {capturedSnapshots.length > 0 ? (
              capturedSnapshots.map(snap => {
                ...
                return (
                  <div key={`${snap.id}-${snap.timestamp}`} className={`flex items-start gap-4 border rounded-xl p-2 transition-colors shadow-sm ${isKnown ? 'border-purple-500/40 bg-purple-500/5 hover:border-purple-500/80' : 'border-accent/40 bg-accent-soft hover:border-accent'}`}>
                    ...
                    <div className="flex-1 flex flex-col gap-1 justify-center min-w-0">
                      <div className="flex justify-between items-center gap-2">
                        <span className={`text-[10px] font-black truncate tracking-wider ${isKnown ? 'text-purple-500' : 'text-accent'}`}>
                      ...
                      <div className="flex items-center justify-between mt-1">
                        ...
                        {!isKnown && (
                          <button 
                            ...
                            className="flex items-center gap-1 text-[10px] font-bold text-background bg-accent hover:bg-accent-hover px-2 py-1 rounded-md shadow-sm transition-colors"
                          >
                            <UserPlus size={12} />
                            <span>Tanıt</span>
                          </button>
