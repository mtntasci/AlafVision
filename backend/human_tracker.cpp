...
int HumanTrackerInit() {
  try {
    net = readNetFromDarknet("models/yolov4-tiny.cfg",
                             "models/yolov4-tiny.weights");
    net.setPreferableBackend(DNN_BACKEND_OPENCV);
    net.setPreferableTarget(DNN_TARGET_CPU);
    net_initialized = true;
    std::cout
        << "[HumanTracker] YOLOv4-tiny initialized successfully via OpenCV DNN."
        << std::endl;
    return 0;
  } catch (const std::exception &e) {
    std::cerr << "[HumanTracker] Error loading YOLO model: " << e.what()
              << std::endl;
    return -1;
  }
}
...
char *HumanTrackerProcessFrame(const void *data, int size) {
  ...
  Mat blob = blobFromImage(frame, 1 / 255.0, Size(416, 416), Scalar(0, 0, 0),
                           true, false);
  net.setInput(blob);
  ...
  for (size_t i = 0; i < current_detections.size(); ++i) {
    ...
    float min_dist =
        300.0f; // Extremely generous threshold to prevent ID flickering
  ...
  for (size_t i = 0; i < current_detections.size(); ++i) {
    ...
    json += "\"box\": [" + std::to_string(b.x) + ", " + std::to_string(b.y) +
            ...
            "], ";
    json += "\"class\": \"person\", \"score\": " +
            std::to_string(current_scores[i]) + "}";
